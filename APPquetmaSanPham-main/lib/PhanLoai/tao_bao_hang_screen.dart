...
class _CreateTOState extends State<CreateTO>
  ...
  Future<void> _updateTOInDatabase() async {
    await TODatabase.instance.updateTO(
      TOModel(
        maTO: toId,
  ...
  void initState() {
    ...
    if (editTO != null) {
      ...
      scannedCodes.addAll(
        editTO.danhSachGoiHang.asMap().entries.map(
          (entry) {
            int idx = entry.key;
  ...
  Future<void> _processCode(String code, String codeType) async {
    ...
    if (order == null) {
      ...
      await _playErrorSound();
      print('❌ Lý do: Mã không tồn tại trong database (orders.db)');
    ...
    if (_stationBanDau.isEmpty) {
      ...
      addressController.text = orderStation;
    ...
    if (orderStation != _stationBanDau) {
      _showCenterMessage(
        'Đơn này đi $orderStation, TO này chỉ nhận $station. Từ chỏi!',
        Colors.red,
      );
      await _playErrorSound();
    ...
    print('⚖️ Kiểm tra trọng lượng: $TongKhoiLuong + $itemWeight = ${TongKhoiLuong + itemWeight} / ${TOModel.maxWeight}');
    if (TongKhoiLuong + itemWeight > TOModel.maxWeight) {
      _showCenterMessage(
        'Thêm mã này sẽ vượt 10KG! TO đầy rồi.',
        Colors.red,
      );
      await _playErrorSound();
    ...
    print('📦 Kiểm tra số lượng: ${scannedCodes.length} / ${TOModel.maxGoiHang}');
    ...
    print('   - Số lượng hiện tại: ${scannedCodes.length} / ${TOModel.maxGoiHang}');
    print('   - Trọng lượng hiện tại: $TongKhoiLuong / ${TOModel.maxWeight} kg');
  ...
  Future<void> _completeAndCloseTo() async {
    ...
    // Cập nhật status thành 'Packed' trong database
    await TODatabase.instance.updateTO(
      TOModel(
        maTO: toId,
        danhSachGoiHang: scannedCodes.map((item) => item.code).toList(),
